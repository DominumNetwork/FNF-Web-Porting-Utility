...
app.post('/build', upload.single('modZip'), (req, res) => {
  ...
  setTimeout(() => {
    try {
      ...
      execSync(\`find source -name "*.hx" -type f -exec sed -i -e 's/sys\\\\.FileSystem/mock.FileSystem/g' -e 's/sys\\\\.io\\\\.File/mock.File/g' -e 's/sys\\\\.io\\\\.Process/mock.Process/g' -e 's/sys\\\\.thread\\\\.Thread/mock.Thread/g' -e 's/sys\\\\.thread\\\\.Mutex/mock.Mutex/g' -e 's/\\\\bSys\\\\./mock.Sys./g' -e 's/import Sys;/import mock.Sys;/g' -e '/import cpp\\\\./d' -e 's/cpp\\\\.ConstCharStar/String/g' -e 's/cpp\\\\.Callable/Dynamic/g' -e 's/^package.*/&\\nimport mock.FileSystem;\\nimport mock.File;\\nimport mock.Process;\\nimport mock.Thread;\\nimport mock.Mutex;\\nimport mock.Sys;/g' {} +\`, { cwd: \`\${workDir}/engine-source\` });
      execSync(\`find source -name "FlxSoundTray.hx" -type f -exec sed -i 's/public function new(/public function showIncrement() { show(); } public function showDecrement() { show(); }\\n\\n&/g' {} +\`, { cwd: \`\${workDir}/engine-source\` });
