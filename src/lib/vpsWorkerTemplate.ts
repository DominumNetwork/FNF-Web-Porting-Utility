...
app.post('/build', upload.single('modZip'), (req, res) => {
  ...
  setTimeout(() => {
    try {
      ...
      fs.writeFileSync(\`\${workDir}/engine-source/source/Process.hx\`, 'class Process { public function new(c:String, ?a:Dynamic) {} public function exitCode(b:Bool=true) return 0; public function close() {} }');
