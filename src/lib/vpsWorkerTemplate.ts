...
app.post('/build', upload.single('modZip'), (req, res) => {
  ...
  setTimeout(() => {
    try {
      ...
      execSync(\`find . -name "*.hx" -type f -exec sed -i -e 's/sys\\\\.FileSystem/FileSystem/g' -e 's/sys\\\\.io\\\\.File/File/g' -e 's/sys\\\\.io\\\\.Process/Process/g' -e 's/sys\\\\.thread\\\\.Thread/Thread/g' -e 's/sys\\\\.thread\\\\.Mutex/Mutex/g' -e '/import cpp\\\\./d' -e 's/cpp\\\\.ConstCharStar/String/g' -e 's/cpp\\\\.Callable/Dynamic/g' {} +\`, { cwd: \`\${workDir}/engine-source\` });
